use std::{
	error::Error,
	io::Write,
	path::Path,
	sync::{
		atomic::{AtomicBool, AtomicUsize, Ordering},
		Arc,
	},
	time::Duration,
};

use geo::{
	map_index_to_lat_lon,
	Dataset,
	DatasetBuilder,
	Hillshade,
	HillshadeBuilder,
	HillshadeMetadata,
	TileMetadata,
};
use rayon::prelude::*;

pub fn for_tile_in_output(
	output: &Path, compression_level: i8, dictionary: &[u8], metadata: TileMetadata,
	exec: impl Fn(i16, i16, &DatasetBuilder) -> Result<(), Box<dyn Error>> + Sync,
) {
	let was_quit = Arc::new(AtomicBool::new(false));
	let handler_used = was_quit.clone();
	let was_quit = &was_quit;

	let _ = ctrlc::set_handler(move || {
		if handler_used.load(Ordering::Acquire) {
			std::process::exit(1);
		}

		println!("\nFinishing up, press Ctrl + C again to exit immediately (will result in some data loss)");
		handler_used.store(true, Ordering::Release);
	});

	fn make_builder(
		path: &Path, metadata: TileMetadata, compression_level: i8, dictionary: &[u8],
	) -> Result<DatasetBuilder, std::io::Error> {
		if let Ok(x) = Dataset::load(path) {
			if metadata == x.metadata() {
				println!("Continuing from last execution");
				return DatasetBuilder::from_dataset(&path, x, compression_level, dictionary);
			}
		}
		DatasetBuilder::new(&path, metadata, compression_level, dictionary)
	}

	let builder = match make_builder(&output, metadata, compression_level, dictionary) {
		Ok(x) => x,
		Err(e) => {
			eprintln!("{}", e);
			return;
		},
	};
	let rbuilder = &builder;

	let tiles = 360 * 180;
	let counter = AtomicUsize::new(1);
	let had_error = AtomicBool::new(false);
	let had_error = &had_error;

	let _ = crossbeam::scope(move |scope| {
		scope.spawn(move |_| {
			while !was_quit.load(Ordering::Acquire) {
				std::thread::sleep(Duration::from_secs(10));
				let _ = rbuilder.flush();
			}
		});

		print!("\r{}/{}", counter.load(Ordering::Relaxed), tiles);
		(0..tiles).into_par_iter().for_each(|index| {
			tracy::zone!("Process tile");
			if had_error.load(Ordering::Acquire) || was_quit.load(Ordering::Acquire) {
				return;
			}

			let (lat, lon) = map_index_to_lat_lon(index);
			if !rbuilder.tile_exists(lat, lon) {
				match exec(lat, lon, &rbuilder) {
					Ok(_) => {},
					Err(e) => {
						println!("Error in tile {}, {}: {}", lat, lon, e);
						had_error.store(true, Ordering::Release);
					},
				}
			}

			print!("\r{}/{}", counter.fetch_add(1, Ordering::Relaxed), tiles);
			let _ = std::io::stdout().flush();
		});

		was_quit.store(true, Ordering::Release);
	});

	(!had_error.load(Ordering::Relaxed))
		.then(|| builder.finish())
		.map(|x| match x {
			Ok(_) => {},
			Err(e) => println!("Error saving output: {}", e),
		});
}

pub fn for_tile_in_hillshade_output(
	output: &Path, compression_level: i8, metadata: HillshadeMetadata,
	exec: impl Fn(i16, i16, &HillshadeBuilder) -> Result<(), Box<dyn Error>> + Sync,
) {
	let was_quit = Arc::new(AtomicBool::new(false));
	let handler_used = was_quit.clone();
	let was_quit = &was_quit;

	let _ = ctrlc::set_handler(move || {
		if handler_used.load(Ordering::Acquire) {
			std::process::exit(1);
		}

		println!("\nFinishing up, press Ctrl + C again to exit immediately (will result in some data loss)");
		handler_used.store(true, Ordering::Release);
	});

	fn make_builder(
		path: &Path, metadata: HillshadeMetadata, compression_level: i8,
	) -> Result<HillshadeBuilder, std::io::Error> {
		if let Ok(x) = Hillshade::load(path) {
			if metadata == x.metadata() {
				println!("Continuing from last execution");
				return HillshadeBuilder::from_dataset(&path, x, compression_level);
			}
		}
		HillshadeBuilder::new(&path, metadata, compression_level)
	}

	let builder = match make_builder(&output, metadata, compression_level) {
		Ok(x) => x,
		Err(e) => {
			eprintln!("{}", e);
			return;
		},
	};
	let rbuilder = &builder;

	let tiles = 360 * 180;
	let counter = AtomicUsize::new(1);
	let had_error = AtomicBool::new(false);
	let had_error = &had_error;

	let _ = crossbeam::scope(move |scope| {
		scope.spawn(move |_| {
			while !was_quit.load(Ordering::Acquire) {
				std::thread::sleep(Duration::from_secs(10));
				let _ = rbuilder.flush();
			}
		});

		print!("\r{}/{}", counter.load(Ordering::Relaxed), tiles);
		(0..tiles).into_par_iter().for_each(|index| {
			tracy::zone!("Process tile");
			if had_error.load(Ordering::Acquire) || was_quit.load(Ordering::Acquire) {
				return;
			}

			let (lat, lon) = map_index_to_lat_lon(index);
			if !rbuilder.tile_exists(lat, lon) {
				match exec(lat, lon, &rbuilder) {
					Ok(_) => {},
					Err(e) => {
						println!("Error in tile {}, {}: {}", lat, lon, e);
						had_error.store(true, Ordering::Release);
					},
				}
			}

			print!("\r{}/{}", counter.fetch_add(1, Ordering::Relaxed), tiles);
			let _ = std::io::stdout().flush();
		});

		was_quit.store(true, Ordering::Release);
	});

	(!had_error.load(Ordering::Relaxed))
		.then(|| builder.finish())
		.map(|x| match x {
			Ok(_) => {},
			Err(e) => println!("Error saving output: {}", e),
		});
}
